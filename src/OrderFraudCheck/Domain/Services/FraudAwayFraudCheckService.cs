...
public class FraudAwayFraudCheckService : IFraudCheckService
{
    ...
    private readonly RiskScoreOptions _riskScoreOptions;

    public FraudAwayFraudCheckService(IFraudCheckService nextFraudCheckService, IFraudAwayProvider? fraudAwayProvider, ISaveFraudAwayDetailsCommand? saveFraudAwayDetailsCommand, RiskScoreOptions riskScoreOptions)
    {
        ...
        _riskScoreOptions = riskScoreOptions ?? throw new ArgumentNullException(nameof(riskScoreOptions));
    }
    ...
    {
        return fraudCheckAwayResponse!.FraudRiskScore < _riskScoreOptions.RiskScoreThreshold ? FraudCheckStatus.Passed : FraudCheckStatus.Failed;
    }
